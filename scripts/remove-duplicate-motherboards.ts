...
async function removeDuplicates() {
  try {
    ...
    const motherboardsByName: Record<string, typeof motherboards> = {};
    ...
    // Trigger a backup after cleanup
    try {
      const response = await fetch('http://localhost:5000/api/components/backup', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      });
      
      if (response.ok) {
        console.log('Components successfully backed up to CSV after cleanup');
      } else {
        console.error('Failed to backup components after cleanup');
      }
    } catch (backupError) {
      console.error('Error triggering backup:', backupError);
    }
